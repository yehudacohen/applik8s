...
${exposureSnippet}`;
  ...
  const publicUrl = input.publicUrl ?? "http://guestbook.localhost/";
  const managedHttps = publicUrl.startsWith("https://");
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .demo-description { max-width: 920px; color: #e6d5be; }
      .demo-description strong { display: inline; margin-right: 8px; color: #fffaf2; font-size: inherit; }
      ...
      .exposure-panel { display: grid; grid-template-columns: minmax(0, 1.25fr) minmax(280px, .75fr); gap: 24px; align-items: center; border-left: 10px solid #2f7652; }
      .exposure-contract { display: grid; gap: 10px; padding: 20px; border: 1px solid #c69a72; border-radius: 18px; background: #24170f; color: #fffaf2; }
      .exposure-contract span { color: #e5bf91; font: 900 12px/1.2 ui-monospace, SFMono-Regular, Menlo, monospace; text-transform: uppercase; letter-spacing: .1em; }
      .exposure-contract strong { color: #fffaf2; font-size: 18px; }
      .exposure-contract code { color: #f0b36d; }
      ...
      @media (max-width: 1120px) { .guestbook, .context-grid, .exposure-panel, #why-applik8s .section-head, .about-head { grid-template-columns: 1fr; } .form-card { position: static; } .steps, ol, .runtime-cards, .link-cards, #why-applik8s .link-cards { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
    ...
    <main>
      ...
      <header class="hero">
        <div>
          ...
          <p class="demo-description"><strong>${escapeMarkup(input.title)}</strong>${escapeMarkup(input.description)}</p>
      ...
      <section class="panel exposure-panel" id="exposure" aria-labelledby="exposure-title" data-exposure-mode="${managedHttps ? "managed-https" : "local-http"}">
        <div class="section-head"><p class="eyebrow">One deployment intent</p><h2 id="exposure-title">The UI and its public edge come from the same application graph.</h2><p><code>app.expose()</code> targets the generated web Service. The local profile stays dependency-free over HTTP; the public profile adds an Ingress, a namespaced cert-manager <code>Certificate</code>, and ExternalDNS intent without pretending that certificate issuance or DNS propagation completed merely because the resources were accepted.</p></div>
        <div class="exposure-contract"><span>${managedHttps ? "Public profile · managed HTTPS requested" : "Local profile · HTTP only"}</span><strong>${escapeMarkup(publicUrl)}</strong><code>app.expose("web", { service: web, tls, dns })</code></div>
      </section>
      <section class="panel code-tour" id="code-tour" aria-labelledby="code-tour-title">
        ...
        <div class="runtime-cards" aria-label="Generated runtime pieces">
          ...
          <div><strong>Public edge</strong><p>Generates Ingress, managed Certificate, and explicit DNS intent for the public profile.</p></div>
        </div>
      </section>
  ...
  handlers: [
    GuestBook.on.reconcile(async (book) => {
      ...
      const renderGuestBookSnapshot = (input: {
        ...
        const exposureSnippet = 'app.provide(Certificate, Certificate.certManager({\\n  issuerRef: { name: "letsencrypt-prod", kind: "ClusterIssuer" },\\n}));\\napp.provide(DnsPublication, DnsPublication.externalDns());\\n\\napp.expose("web", {\\n  service: web,\\n  hostnames: ["guestbook.example.com"],\\n  tls: { mode: "managed" },\\n  dns: { mode: "managed", ttlSeconds: 120 },\\n});';
        const lineBreak = String.fromCharCode(10);
        const formattedStackSnippet = `${stackSnippet}\\n\\n${exposureSnippet}`
          .replaceAll("\\n", lineBreak)
          .replaceAll(`${lineBreak}+ `, lineBreak);
        const pageTitle = "This Website Was Rendered by the Kubernetes Control Plane";
        const publicUrl = input.publicUrl ?? "http://guestbook.localhost/";
        const managedHttps = publicUrl.startsWith("https://");
        let html = "";
        ...
        html += '<style>.exposure-panel{display:grid;grid-template-columns:minmax(0,1.25fr) minmax(280px,.75fr);gap:24px;align-items:center;border-left:10px solid #2f7652}.exposure-contract{display:grid;gap:10px;padding:20px;border:1px solid #c69a72;border-radius:18px;background:#24170f;color:#fffaf2}.exposure-contract span{color:#e5bf91;font:900 12px/1.2 ui-monospace,SFMono-Regular,Menlo,monospace;text-transform:uppercase;letter-spacing:.1em}.exposure-contract strong{color:#fffaf2;font-size:18px}.exposure-contract code{color:#f0b36d}@media(max-width:1120px){.exposure-panel{grid-template-columns:1fr}}</style>';
        ...
        html += '<style>.demo-description{max-width:920px;color:#e6d5be}.demo-description strong{display:inline;margin-right:8px;color:#fffaf2;font-size:inherit}</style>';
        ...
        html += `<section class="panel exposure-panel" id="exposure" data-exposure-mode="${managedHttps ? "managed-https" : "local-http"}"><div class="section-head"><p class="eyebrow">One deployment intent</p><h2>The UI and its public edge come from the same application graph.</h2><p><code>app.expose()</code> targets the generated web Service. The local profile stays dependency-free over HTTP; the public profile adds an Ingress, a namespaced cert-manager <code>Certificate</code>, and ExternalDNS intent without pretending issuance or propagation completed merely because resources were accepted.</p></div><div class="exposure-contract"><span>${managedHttps ? "Public profile · managed HTTPS requested" : "Local profile · HTTP only"}</span><strong>${escapeMarkup(publicUrl)}</strong><code>app.expose("web", { service: web, tls, dns })</code></div></section>`;
        html += `<section class="panel code-tour" id="code-tour"><div class="section-head"><p class="eyebrow">Source tour</p><h2>One source file becomes the stack.</h2><p>The same TypeScript source produces the UI server, operator handler, CRDs, indexer, Ingress, and managed edge intent.</p></div><article class="stack-code"><pre><code class="language-ts">${escapeMarkup(formattedStackSnippet)}</code></pre></article><div class="runtime-cards"><div><strong>Web server</strong><p>Serves the rendered status snapshot and accepts form posts.</p></div><div><strong>Operator</strong><p>Publishes entries and patches the rendered GuestBook status.</p></div><div><strong>Indexer</strong><p>Queries older entries without growing the front-page snapshot.</p></div><div><strong>CRDs</strong><p>Make GuestBook and GuestBookEntry visible as Kubernetes objects.</p></div><div><strong>Public edge</strong><p>Generates Ingress, managed Certificate, and explicit DNS intent for the public profile.</p></div></div></section>`;
