...
function Moderation() {
  ...
  return <ChirpShell title="Moderation">
    <PageIntro eyebrow="Administrator evidence" title="Moderation queue">Reports, cases, and desired moderation policy are durable relational models. Applik8s continuously reconciles the policy without exposing its runtime provider to this UI.</PageIntro>
    <section className="panelForm" aria-labelledby="moderation-policy-heading"><p className="eyebrow">Managed desired state</p><h2 id="moderation-policy-heading">Moderation policy</h2><QueryState phase={moderationPolicy.phase} error={moderationPolicy.error} empty={(moderationPolicy.data?.length ?? 0) === 0} />{moderationPolicy.data?.map((current) => <p key={current.name}>Maximum risk {current.maxRisk} · {current.blockedTerms.length} blocked terms.</p>)}</section>
