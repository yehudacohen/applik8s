...
import { queryCacheKey } from '@applik8s/client';
import { runApplicationProjection, type ApplicationProjectionStore, type ApplicationStreamEnvelope } from '@applik8s/applik8s';

const root = new URL('..', import.meta.url).pathname.replace(/\/$/, '');
const budgets = JSON.parse(await readFile(join(root, 'benchmarks/v0.6/budgets.json'), 'utf8')) as { readonly client: { readonly minimumCacheKeysPerSecond: number }; readonly projection: { readonly minimumEventsPerSecond: number; readonly maximumRssGrowthBytes: number } };
...
const report = { schemaVersion: 2, release: 'v0.6', evidenceClass: 'synthetic-local', limitations: ['No network, PostgreSQL, ClickHouse, Kubernetes scheduler, or multi-process contention is measured by this microbenchmark.', 'Datastore and cluster performance require the explicit live benchmark/evidence lanes.'], generatedAt: new Date().toISOString(), operations, client: { durationMs: round(clientDurationMs), cacheKeysPerSecond: round(operations / (clientDurationMs / 1000)) }, projection: { durationMs: round(projectionDurationMs), eventsPerSecond: round(operations / (projectionDurationMs / 1000)), rssGrowthBytes: Math.max(0, process.memoryUsage().rss - memoryBefore) } };
const violations = [report.client.cacheKeysPerSecond < budgets.client.minimumCacheKeysPerSecond ? 'client cache-key throughput' : '', report.projection.eventsPerSecond < budgets.projection.minimumEventsPerSecond ? 'projection throughput' : '', report.projection.rssGrowthBytes > budgets.projection.maximumRssGrowthBytes ? 'projection RSS growth' : ''].filter(Boolean);
