...
const container = `applik8s-v08-ministack-${process.pid}`;
let started = false;
...
try {
  ...
  const child = Bun.spawn([
    ...
    '--maxWorkers=1',
    'packages/e2e/test/v08-aws-local-ministack-live.e2e.test.ts',
  ], {
    cwd: process.cwd(),
    env: {
      ...process.env,
      ...
      APPLIK8S_MINISTACK_CONTAINER: container,
    },
    stdin: 'inherit',
    stdout: 'inherit',
    stderr: 'inherit',
...
async function waitForMiniStack(endpoint: string): Promise<void> {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      const edition = health && typeof health === 'object' ? Reflect.get(health, 'edition') : undefined;
      if (response.ok && version === '1.4.20' && edition === 'full') return;
    } catch {
      // The loopback listener is intentionally absent during startup.
    }
    await new Promise((resolveDelay) => setTimeout(resolveDelay, 250));
  }
  throw new Error('Pinned MiniStack 1.4.20-full did not become healthy within 60 seconds.');
}
