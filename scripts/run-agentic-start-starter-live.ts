...
await createApplicationAgenticStart({
  ...
  async run(command) {
    ...
    await writeOfficialTanStackScaffold(target);
  },
});
...
async function writeOfficialTanStackScaffold(directory: string): Promise<void> {
...
`,
  ...
  // Preserve an already generated migration history between reruns. A fresh
  // target starts empty and drizzle-kit materializes the authoritative schema.
