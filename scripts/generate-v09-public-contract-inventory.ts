...
const diagnosticRegistryPath = resolve(
  root,
  'packages/compiler/src/diagnostics/public-diagnostic-registry.ts',
);
const diagnosticDocumentationPath = resolve(
  root,
...
const diagnosticRegistry = renderDiagnosticRegistry(diagnostics);
const diagnosticDocumentation = renderDiagnosticDocumentation(diagnostics);
if (process.argv.includes('--write')) {
  await writeFile(outputPath, serialized, 'utf8');
  await writeFile(diagnosticRegistryPath, diagnosticRegistry, 'utf8');
  await writeFile(
  ...
  const currentDiagnosticRegistry = await readFile(
    diagnosticRegistryPath,
    'utf8',
  ).catch(() => '');
  if (currentDiagnosticRegistry !== diagnosticRegistry) {
    throw new Error(
      'PUBLIC_DIAGNOSTIC_REGISTRY_DIRTY: run bun run generate:v09:public-contracts and commit the result.',
    );
  }
  const currentDiagnosticDocumentation = await readFile(
    diagnosticDocumentationPath,
    'utf8',
  ).catch(() => '');
  if (currentDiagnosticDocumentation !== diagnosticDocumentation) {
    throw new Error(
      'PUBLIC_DIAGNOSTIC_DOCUMENTATION_DIRTY: run bun run generate:v09:public-contracts and commit the result.',
    );
  }
...
function collectDiagnostics(): readonly {
  ...
  for (const source of program.getSourceFiles()) {
    ...
    if (
      ...
      || path.includes('/dist/')
      || path.includes('/test/')
      || path === 'packages/compiler/src/diagnostics/public-diagnostic-registry.ts'
    ) continue;
    visit(source, source);
  }
  return [...sourcesByCode.entries()].map(([code, sources]) => {
    ...
    return {
      ...
      stability: disposition.stability,
      documentation: `docs-site/src/content/docs/reference/diagnostics.mdx#${diagnosticAnchor(code)}`,
      evidence: [...disposition.evidence],
    };
  });
...
function diagnosticAnchor(code: string): string {
  return `diagnostic-${code.toLowerCase().replaceAll('_', '-')}`;
}

function renderDiagnosticRegistry(
  ...
  return `// Generated by scripts/generate-v09-public-contract-inventory.ts.
// Do not edit by hand.
...
export const publicDiagnosticRecords = Object.freeze(${JSON.stringify(
    diagnostics.map(({ code, owner, sources, documentation }) => ({
      code,
      owner,
      sources,
      documentation,
...
function renderDiagnosticDocumentation(
  diagnostics: readonly {
    readonly code: string;
    readonly owner: string;
    readonly sources: readonly string[];
  ...
  const entries = diagnostics.map(({ code, owner, sources }) => `
<a id="${diagnosticAnchor(code)}"></a>

## \`${code}\`

- Owner: \`${owner}\`
- Defining source${sources.length === 1 ? '' : 's'}: ${sources.map((source) => `\`${source}\``).join(', ')}
- Explain: \`applik8s explain ${code}\`
