...
const deployArgs = [
  ...
  ...(options.planFormat ? ['--plan-format', String(options.planFormat)] : []),
  ...(options.planDiff ? ['--plan-diff', resolve(cwd, String(options.planDiff))] : []),
  ...(options.development ? ['--development'] : []),
  ...(options.allowBreakingChanges ? ['--allow-breaking-changes'] : []),
  ...((options.acknowledge ?? []).flatMap((token) => ['--acknowledge', String(token)])),
];
const deleteArgs = [
  '--enable-source-maps',
  '--import', pathToFileURL(registerTypescript).href,
