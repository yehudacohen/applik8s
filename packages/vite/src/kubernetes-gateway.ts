...
import type { ApplicationCommandProgress, ApplicationCommandSubmission, ApplicationQueryEvent, ApplicationQuerySnapshot } from '@applik8s/client';
...
export interface Applik8sKubernetesGatewayOptions {
  readonly authenticate: (request: Request) => Applik8sGatewayAdmission | Promise<Applik8sGatewayAdmission>;
...
export function createApplik8sKubernetesGateway(options: Applik8sKubernetesGatewayOptions): Applik8sKubernetesGateway {
  ...
  async function submitCommand(request: Request, command: Applik8sKubernetesCreateContract, body: Readonly<Record<string, unknown>>): Promise<Response> {
    const admission = await admit(options.authenticate, request);
    ...
    const namespace = command.resource.scope === 'Namespaced' ? requiredString(placement.namespace, 'placement.namespace') : undefined;
...
async function listSnapshot(
  ...
  const namespace = query.resource.scope === 'Namespaced'
    ? requiredString(query.fixedNamespace ?? query.namespace?.({ context, input }), 'query.namespace')
    : undefined;
...
async function admit(authenticate: Applik8sKubernetesGatewayOptions['authenticate'], request: Request): Promise<Applik8sGatewayAdmission> {
