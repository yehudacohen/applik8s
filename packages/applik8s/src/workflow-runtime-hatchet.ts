...
function applicationMetadata(metadata: ApplicationWorkflowInvocationMetadata | undefined): Record<string, string> {
  const trustedContext = metadata?.trustedContext ? JSON.stringify(metadata.trustedContext) : undefined;
  if (trustedContext && Buffer.byteLength(trustedContext) > 8_192) throw new Error('Workflow trusted context exceeds the bounded 8192-byte durable metadata limit.');
