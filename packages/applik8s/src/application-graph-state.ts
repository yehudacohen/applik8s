...
export function applicationGraphFromState(name: string, state: ApplicationGraphState): ApplicationGraph {
  ...
  return normalizeApplicationGraph({
    ...
    compatibility: {
      ...
      experimentalSurfaces: ['app.graph', 'app.job', 'app.transaction.saga', 'provider.JobRuntime'],
      ...
      labels: [
        ...
        { name: 'app.transaction.saga', surface: 'experimentalSurface', since: 'v0.9', rationale: 'Explicit compensating coordination over distributed effects remains beta until one deployed workflow provider passes the complete interruption and unknown-outcome matrix.', implementation: 'implemented' },
