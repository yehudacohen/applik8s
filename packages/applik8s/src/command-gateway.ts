...
export interface ApplicationCommandGatewayOptions<TPrincipal extends ApplicationQueryPrincipal = ApplicationQueryPrincipal> {
  ...
  readonly authorize: (request: { readonly principal: TPrincipal; readonly command: string; readonly input: unknown }) => boolean | Promise<boolean>;
    ...
    async handle(request) {
      ...
      try {
        ...
        if (route.operation === 'submit') {
          ...
          if (!await options.authorize({ principal: admission.principal as TPrincipal, command: command.id, input })) return json({ error: 'forbidden' }, 403);
          ...
          const targetKey = canonicalApplicationCommandKey(command.key(input) as string | number | boolean | Readonly<Record<string, string | number | boolean>>);
          ...
          const envelope = { id: commandId, contract: commandContract(command.id), payload: input, recordedAt: now().toISOString(), correlationId, partitionKey: targetKey, routing: { binding: command.bindingId, targetKey, idempotencyKey: command.idempotencyKey?.(input) ?? idempotencyKey }, ...(typeof body.value.expectedRevision === 'string' ? { expectedRevision: body.value.expectedRevision } : {}), trustedContext: { values: admission.trustedContext, digest: contextDigest } };
          await publisher.publish(envelope, 'commands');
          const cursor = encodeCursor(options.cursorSecret, { version: 1, command: command.id, commandId, correlationId, bindingId: command.bindingId, model: command.model, targetKey, idempotencyKey: envelope.routing.idempotencyKey, principalId: admission.principal.id, authorizationVersion: admission.authorizationVersion, contextDigest, expiresAt: now().getTime() + cursorTtlSeconds * 1000 });
        ...
        if (row.error) return json({ ...progress(cursor, encoded, 'rejected'), rejection: durableRejection(row.error), ...(typeof row.model_revision === 'string' ? { modelRevision: row.model_revision } : {}) }, 200);
...
function progress(cursor: ProgressCursor, encoded: string, durableResult: 'pending' | 'succeeded' | 'rejected'): ApplicationCommandProgress { return { protocol: 'applik8s.command/v1alpha1', command: cursor.command, commandId: cursor.commandId, correlationId: cursor.correlationId, transport: 'acknowledged', durableResult, progressCursor: encoded, workflow: 'notStarted', reconciliation: durableResult === 'succeeded' ? 'progressing' : 'notObserved' }; }
...
function decodeCursor(secret: string, value: string, now: number): ProgressCursor { const [body, signature, extra] = value.split('.'); if (!body || !signature || extra) throw new Error('Application command cursor is invalid.'); const expected = createHmac('sha256', secret).update(body).digest(); const supplied = Buffer.from(signature, 'base64url'); if (supplied.length !== expected.length || !timingSafeEqual(supplied, expected)) throw new Error('Application command cursor is invalid.'); const cursor = JSON.parse(Buffer.from(body, 'base64url').toString('utf8')) as ProgressCursor; if (cursor.version !== 1 || cursor.expiresAt < now) throw new Error('Application command cursor is invalid or expired.'); return cursor; }
