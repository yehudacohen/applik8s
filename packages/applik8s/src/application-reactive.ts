...
function recordProjectionProvider(
  ...
  const node: ApplicationProviderNode<'AnalyticalDatabase'> = {
    ...
    config: { provider: 'clickhouse', ...(qualification ? { qualification: qualification as unknown as JsonValue } : {}), enabled: provider.enabled ?? true, name: provider.name ?? 'applik8s-analytics', namespace: provider.namespace ?? 'applik8s-analytics', provision: provider.provision ?? true, endpoint: (provider.endpoint ?? applicationTypeKroString('http://clickhouse-', provider.name ?? 'applik8s-analytics', '.', provider.namespace ?? 'applik8s-analytics', '.svc.cluster.local:8123')) as unknown as JsonValue, database: (provider.database ?? 'default') as unknown as JsonValue, ...(provider.credentialsSecret?.name ? { credentialsSecret: { apiVersion: provider.credentialsSecret.apiVersion, kind: provider.credentialsSecret.kind, name: provider.credentialsSecret.name, ...(provider.credentialsSecret.namespace ? { namespace: provider.credentialsSecret.namespace } : {}) }, usernameKey: provider.usernameKey ?? 'username', passwordKey: provider.passwordKey ?? 'password' } : {}), ...(provider.externalConnection ? { externalConnection: provider.externalConnection as unknown as JsonValue } : {}) },
  };
  addApplicationGraphNode(state, node);
  return { interface: 'AnalyticalDatabase', nodeId };
}
