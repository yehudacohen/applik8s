...
function commandProcessorBindingFor(
  ...
  let index = bindingIndexes.get(bindings);
  if (!index) {
    index = new Map(bindings.map((binding) => [`${binding.contract.name}\u0000${binding.contract.version}`, binding]));
    bindingIndexes.set(bindings, index);
  }
  return index.get(`${contract.name}\u0000${contract.version}`);
}
