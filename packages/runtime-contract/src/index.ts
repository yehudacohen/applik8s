      ...
      runtime: objectSchema(
        {
          ...
          identityEnvelope: objectSchema({
            apiVersion: constSchema('applik8s.guestHostIdentity/v1alpha1'),
            application: { type: 'string', pattern: '^applik8s://' },
            operation: { type: 'string', pattern: '^applik8s://' },
            execution: { type: 'string', pattern: '^applik8s://' },
            artifact: { type: 'string', pattern: '^applik8s://' },
            attempt: { type: 'string', minLength: 1 },
            runtimeAccess: objectSchema({
              version: constSchema('v1alpha1'),
              digest: { type: 'string', pattern: '^sha256:[a-f0-9]{64}$' },
              requirementIds: { type: 'array', items: { type: 'string' } },
            }, ['version', 'digest', 'requirementIds']),
