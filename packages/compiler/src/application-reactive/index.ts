...
import type { ApplicationCommandHandlerNode, ApplicationCommandNode, ApplicationGatewayNode, ApplicationGraph, ApplicationHandlerDependencies, ApplicationModelNode, ApplicationProjectionNode, ApplicationProviderNode, ApplicationQueryNode, ApplicationReactiveDatabaseRuntimeContract, ApplicationSerializedCallbackContract, ApplicationStreamNode, ApplicationStreamProcessorNode, ApplicationSubscriptionNode } from '@applik8s/core';
...
async function emitProjection(graph: ApplicationGraph, projection: ApplicationProjectionNode, outDir: string): Promise<GeneratedApplicationReactiveArtifact> {
  ...
  if (provider.interface !== 'ProjectionStore' || provider.implementation !== 'clickhouse') throw new Error(`Generated analytical projection ${projection.id} requires one ClickHouse ProjectionStore provider.`);
...
function gatewayAnalyticalProjectionContract(graph: ApplicationGraph, query: ApplicationQueryNode): GatewayAnalyticalProjectionContract {
  ...
  const stream = requiredNode(nodes, projection.source.nodeId, 'stream', projection.id);
  const provider = requiredProvider(nodes, projection.provider.nodeId, projection.id);
  if (provider.interface !== 'ProjectionStore' || provider.implementation !== 'clickhouse') throw new Error(`Generated query ${query.id} analytical authority requires a ClickHouse-compatible ProjectionStore provider.`);
...
import { createClickHouseProjectionStore, createPostgresApplicationStream, enforcePostgresApplicationStreamRetention, runApplicationProjection } from '@applik8s/applik8s/projection-worker-runtime';
...
const store = createClickHouseProjectionStore({ endpoint: requiredEnv('APPLIK8S_CLICKHOUSE_ENDPOINT'), database: requiredEnv('APPLIK8S_CLICKHOUSE_DATABASE'), table: ${JSON.stringify(table)}, projection: ${JSON.stringify(projection.name)}, stream: ${JSON.stringify(`${stream.name}.${stream.version}`)}, schema: schema(${JSON.stringify(projection.output.jsonSchema)}), ...(process.env.APPLIK8S_CLICKHOUSE_USERNAME ? { username: process.env.APPLIK8S_CLICKHOUSE_USERNAME, password: process.env.APPLIK8S_CLICKHOUSE_PASSWORD ?? '' } : {}) });
...
function generatedValkeyProjectionSource(graphName: string, projection: ApplicationProjectionNode, stream: ApplicationStreamNode, _config: Readonly<Record<string, unknown>>): string {
  ...
  return `import { createServer } from 'node:http';
import { createPostgresApplicationStream, createValkeyOnlineProjectionStore, enforcePostgresApplicationStreamRetention, runApplicationProjection } from '@applik8s/applik8s/projection-worker-runtime';
import { callback as project } from './project.generated.js';
