  ...
  try {
    ...
    const manifest: TypeKroCompositionBundleManifest = {
      ...
      spec: {
        ...
        ...(managedModelArtifacts.length > 0 ? { managedModels: managedModelArtifacts.map((artifact) => ({ name: artifact.name, nodeId: artifact.providerId, executionNodeIds: [...artifact.modelIds], modelIds: [...artifact.modelIds], manifest: artifact.manifestPath, source: artifact.sourcePath, digest: artifact.digest, sizeBytes: artifact.sizeBytes, container: typeKroContainerArtifactReference(artifact.container), ...typeKroFrameworkCredentialReferences(artifact.frameworkCredentials) })) } : {}),
