...
function nativeGraph(path: string, digest?: string): ApplicationGraph {
  const model: ApplicationModelNode = {
    ...
    materialization: { mode: 'providerBacked', provider: { interface: 'TransactionalDatabase', nodeId: 'provider.transactional-database' }, backingResources: [], connection: { env: {}, readiness: { dependencies: [], condition: 'ready', timeoutSeconds: 60 } }, runtimeBoundary: { serializedCallbacks: 'generatedRuntimeClient', scriptExecution: 'scriptRuntimeClient' }, reconciliation: { ownership: 'application', schemaDrift: 'failClosed', deletionPolicy: 'retain' } },
