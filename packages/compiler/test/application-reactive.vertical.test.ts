...
describe('generated v0.6 reactive workloads', () => {
  ...
  it('bundles an authenticated HTTP/SSE gateway with Secret-backed PostgreSQL and cursor authority', async () => {
    const graph = reactiveGraph([
      ...
      { id: 'model.card', kind: 'model', name: 'Card', stability: 'stable', runtime: { name: 'Card', tableName: 'cards', provider: 'postgres', database: 'catalog', clusterName: 'catalog', secretName: 'catalog-app', secretKey: 'uri', secretNamespace: 'catalog', connectionEnvName: 'APPLIK8S_DATABASE_CATALOG_URL', constraints: [], indexes: [], retention: { mode: 'retain' } } },
      ...
      { id: 'query.cards.list.v1', kind: 'query', name: 'cards.list', version: 'v1', stability: 'stable', input: schema({ type: 'object', properties: {}, required: [] }), output: schema({ type: 'array', items: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } }), reads: [{ model: { nodeId: 'model.card' } }], authorization: 'application-defined', trustedContext: [], budgets: { timeoutMs: 1000, maxResultBytes: 10000, maxRows: 100 }, snapshotResume: 'resumableInvalidation', incremental: 'invalidation-requery', cursor: 'opaque-query-version-context-scoped', database, authorizationSource: '({ principal }) => principal.id.length > 0', handlerSource: 'async () => []' },
