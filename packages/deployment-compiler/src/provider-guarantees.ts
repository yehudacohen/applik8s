...
const targetImplementationAliases: Readonly<Record<ApplicationDeploymentTargetKind, Readonly<Record<string, Readonly<Record<string, string>>>>>> = {
  ...
  'aws-local': {
    ...
    ApplicationHost: {
      ...
      'aws-application-host': 'ecs-fargate',
    },
    HttpExposure: { ingress: 'alb', 'node-port': 'alb', 'aws-http-exposure': 'alb' },
    DnsPublication: { 'external-dns': 'route53', 'route53-dns-publication': 'route53' },
    Certificate: { 'cert-manager': 'acm', 'acm-certificate': 'acm' },
